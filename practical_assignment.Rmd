...
* `Depth` is very skewed with a mean much larger than the median and a max value much greater that both median, mean and 3rd. quantile. When fitting, it would probably be a good idea to use a log link to control the skewness of `Depth`. 
...
summary(narwhal[,c("Dist.to.shore","OBDA","VeDBA")])
```
