...
We see that adding splines on `ODBA` leads to a better fit. It is not immediately clear which plot belongs to the observed data. Even though the middle plot belongs to the observed data, one could suspect that it would be the top left plot.
...
Noting the values on the scale on the axes, it is quite clear that the observed data doesn't follow the model. We did try to change the model family to negative binomial with a log link, but this resulted in similar problems. 
