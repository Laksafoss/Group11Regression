...
**PLOT : PHASE PROBLEMER**
*Figur 1*: **DESCRIPTION OF PLOT.**
...
We have choosen to aggregrate the data for the following reasons
...
* The binary variables `Call`, `Buzz`, `Click` are all recoreded as 


##### Aggregrating the data
We have choosen to aggregrate the data before analyzing it. Below we explain how we have choosen to aggregrate data and here we list our reasons for aggregrating data.

* The raw dataset has over a million data lines and fitting models on a dataset of this size is quite computationally heavy and plotting becomes slow.
* The binary variables `Call`, `Buzz`, `Click` are all recoreded as a time series of signals, that is, most observations are zero and then there are a few ones when a call or buzz starts. As we are not doing time series analysis it will be hard to model the raw signals. Therefor it is convinient to summarize the number of calls, buzz and click over some time interval. 

We have choosen to aggregrate data based on a new variable called `Divenumber`. `Divenumber` recoreds when the whale is below 10 meters. When the whale is less then 10 meters below the water surface the `Divenumber` is set to `NA` and when it is more then 10 meters from the surface `Divenumber` is set to some integer. The integers are choose such the the first time the whale is below 10 meters `Divenumber`is set to 1, and it is 1 until the whale is above the 10 meter mark again. The next dive is then recoreded as `Divenumber = 2` and so on. We have recoreded a total number of 734 dives for Thor and 1042 dives for Helge.

Based on this new `Divenumber`variable we have created the following variables
