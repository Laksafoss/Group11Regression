...
Let us first look at our potential response variables. 
...
For this purpose we will fit a generalized linear models over our aggregated data sets and estimate the true parameters of our various explanatory variables. 
...
Similarly, we fit a generalized linear model now with the `Strokerate` variable as response:
...
### Missing values
As already mentioned the data set has quite a lot of missing values. In order to examine whether there is some systematize behind which values are missing, we plot the intersection of missing values:
![](figs/Missing_Values_Overview.png)

We see that almost all missing values are present in the `Dist.to.Paamiut` variable, and that there is no clear tendency that other variables are missing simultaneously. Further, in order to check when the missing values appear, we plot the number of missing variables as a function of the phases (except for `Dist.to.Paamiut`):
