...
kable(NAStrokeDf) %>% 
  kable_styling(bootstrap_options = "striped", full_width = F, position = "left") %>%
  group_rows("NA intervals", 1, 3) %>% 
  group_rows("Time range for whole experiment", 4, 5) 
```
...
Which unsurprisingly shows nearly perfect positive correlation between `ODBA` and`VeDBA`. It also shows a weak positive correlation between `Strokerate` and `ODBA`, `VeDBA`.
